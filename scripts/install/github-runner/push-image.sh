...
# secrets/github-runner-token.sops via sops.
#
...
#   - sops
#   - secrets/github-runner-token.sops (SOPS-encrypted, tracked in git)
...
if [[ ! -f "$CONTROL_DIR/secrets/github-runner-token.sops" ]]; then
	echo "ERROR: secrets/github-runner-token.sops not found." >&2
	exit 1
fi

echo "==> Decrypting GHCR PAT from secrets/github-runner-token.sops ..."
